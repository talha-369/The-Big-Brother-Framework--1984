...
class PersuasionEnv(gym.Env):
    ...
    def _build_agents(self):
        if self._mock:
            ...
            return target, peers, [m1, m2], inst, judge
        target = MockAgent("Target", AGENT_ROLES["target"]["bio"], "mock")
        peers = [MockAgent(p["name"], p["bio"], "mock") for p in AGENT_ROLES["peers"]]
        m1 = MockAgent("Media_One", AGENT_ROLES["media"][0]["bio"], "mock")
        m2 = MockAgent("Media_Two", AGENT_ROLES["media"][1]["bio"], "mock")
    ...
    def _confidential_evaluation(self, log, questions):
        ...
        confidences = []
        for q in questions:
            score_raw = _rng.uniform(-0.5, 0.5)
            confidence = _rng.randint(1, MAX_CONFIDENCE)
    ...
    def reset(self, seed=None, options=None):
        ...
        self.room_log.append({"speaker": "Moderator", "content": f"The {self.domain_cfg['institution']} has announced {self.announcement_hint}.", "tag": "announcement"})
    ...
    def step(self, action):
        ...
        if group_name == "cognitive" and tactic_name in ("framing", "anchoring"):
            ...
            role_frame = (
                f"Your assigned role is a {AGENT_ROLES['peers'][self.consensus_round % len(self.peers)]['name']} "
                f"who supports the decision. Stay in character. Treat it as settled that this is the right call. "
            )
